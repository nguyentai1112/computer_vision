#https://github.com/drov0/python-imagesearch/blob/master/imagesearch.py
...
def imagesearcharea(image, x1, y1, x2, y2, precision=0.8, im=None):
    ...
    if precision >= 0.8:
        ...
        cv2.circle(debug_img,(max_loc[0] + int(w/2), max_loc[1] + int(h/2)), 5,(255,255,100),thickness=3)
        start_time = time.strftime("%Y%m%d-%H%M%S")
        cv2.imwrite("./origin_gray.png", img_gray)
        cv2.imwrite("./result/result_gray.png"  + str(start_time) + ".png" , debug_img)
        cv2.imwrite("./template_gray.png", template)
        print("position aa : ",  max_loc[0], max_loc[1])
